...
static te_type te_even(te_type val) {
  ...
  return (val < 0 ? -(static_cast<te_type>(rounded))
                  : static_cast<te_type>(rounded));
}
...
[[nodiscard]]
constexpr static te_type te_less_than_equal_to(te_type val1,
...
[[nodiscard]]
constexpr static te_type te_greater_than_equal_to(te_type val1,
                                                  te_type val2) noexcept {
  return (!std::isfinite(val1) || !std::isfinite(val2))
             ? te_parser::te_nan
             : static_cast<te_type>((val1 >= val2) ? 1 : 0);
...
static te_type te_not(te_type val) {
  return std::isfinite(val)
             ? static_cast<te_type>(!te_parser::number_to_bool(val))
             : te_parser::te_nan;
}
...
static te_type te_round(te_type val, te_type decimalPlaces) // NOLINT
{
  ...
  const size_t adjustedDecimalPlaces{
      !std::isfinite(decimalPlaces)
          ? 0
          : static_cast<size_t>(std::abs(decimalPlaces))};

  const auto decimalPosition =
      static_cast<te_type>(std::pow(10, adjustedDecimalPlaces));
  ...
  if (!useNegativeRound) {
    if (val < 0) {
      return (decimalPosition == 0)
                 ...
                 : std::ceil(static_cast<te_type>(val * decimalPosition) -
                             ROUND_EPSILON) /
                       decimalPosition;
    }
    return (decimalPosition == 0)
               ? std::floor(val + ROUND_EPSILON)
               : std::floor(static_cast<te_type>(val * decimalPosition) +
                            ROUND_EPSILON) /
                     decimalPosition;
  }
  // ROUND(21.5, -1) = 20
  if (val < 0) {
    return std::ceil(static_cast<te_type>(val / decimalPosition) -
                     ROUND_EPSILON) *
           decimalPosition;
  }
  return std::floor(static_cast<te_type>(val / decimalPosition) +
                    ROUND_EPSILON) *
         decimalPosition;
}

[[nodiscard]]
static te_type te_nper(te_type rate, te_type pmt, te_type presentValue,
                       te_type futureValue, te_type type) {
  if (!std::isfinite(rate) || !std::isfinite(pmt) ||
...
[[nodiscard]]
static te_type te_fv(te_type rate, te_type nper, te_type pmt,
                     te_type presentValue, te_type type) {
  ...
  return -(presentValue * powVal +
           (pmt * (1 + (rate * type)) * (powVal - 1) / rate));
}

[[nodiscard]]
static te_type te_pmt(te_type rate, te_type nper, te_type presentValue,
                      te_type futureValue, te_type type) {
  if (!std::isfinite(rate) || !std::isfinite(nper) ||
      !std::isfinite(presentValue)) {
    return te_parser::te_nan;
  }
  if (!std::isfinite(futureValue)) {
...
[[nodiscard]]
static te_type te_ipmt(te_type rate, te_type period, te_type periods,
                       te_type presentValue, te_type futureValue,
                       te_type type) {
  if (!std::isfinite(rate) || !std::isfinite(period) ||
      !std::isfinite(periods) || !std::isfinite(presentValue)) {
    return te_parser::te_nan;
  }
  ...
  const te_type payment =
      te_pmt(rate, periods, presentValue, futureValue, type);
  ...
  const te_type fvAtN = -(presentValue * powVal +
                          payment * (1 + (rate * type)) * (powVal - 1) / rate);
...
[[nodiscard]]
static te_type te_pv(te_type rate, te_type nper, te_type pmt,
                     te_type futureValue, te_type type) {
  if (!std::isfinite(rate) || !std::isfinite(nper) || !std::isfinite(pmt)) {
    return te_parser::te_nan;
  }
...
[[nodiscard]]
static te_type te_ppmt(te_type rate, te_type period, te_type periods,
                       te_type presentValue, te_type futureValue,
                       te_type type) {
  if (!std::isfinite(rate) || !std::isfinite(period) ||
      !std::isfinite(periods) || !std::isfinite(presentValue)) {
  ...
  const te_type interest =
      te_ipmt(rate, period, periods, presentValue, futureValue, type);
...
static te_type te_cumipmt(te_type rate, te_type periods, te_type presentValue,
                          te_type startPeriod, te_type endPeriod,
                          te_type type) {
  if (!std::isfinite(rate) || !std::isfinite(periods) ||
      !std::isfinite(presentValue) || !std::isfinite(startPeriod) ||
      !std::isfinite(endPeriod) || !std::isfinite(type)) {
    return te_parser::te_nan;
  }
  ...
  if (startPeriod < 1.0 || endPeriod < 1.0 || startPeriod > endPeriod ||
      endPeriod > periods) {
    return te_parser::te_nan;
  }
  ...
  for (int64_t period = fromPeriod; period <= toPeriod; ++period) {
    const te_type interestPayment = te_ipmt(rate, static_cast<te_type>(period),
                                            periods, presentValue, 0, type);
...
static te_type te_cumprinc(te_type rate, te_type periods, te_type presentValue,
  ...
  for (auto period = static_cast<int64_t>(startPeriod);
    ...
    const te_type principal = te_ppmt(rate, static_cast<te_type>(period),
                                      periods, presentValue, 0, type);
...
static te_type te_asset_depreciation(te_type cost, te_type salvage,
  ...
  // month in the first year of depreciation is optional and defaults to a full
  // year
  if (!std::isfinite(month)) {
    month = NUM_OF_MONTHS;
  }
  if (month < 1 || month > NUM_OF_MONTHS || life <= 0 || cost <= 0 ||
      period < 1 || period >= (life + 2)) {
    return te_parser::te_nan;
  }
  ...
  // we just verified that this are integral, but round down to fully ensure
  // that
  ...
  return (period == life + 1)
             ? ((cost - priorDepreciation) * rate * (NUM_OF_MONTHS - month)) /
                   NUM_OF_MONTHS
...
[[nodiscard]]
static te_type
te_fac(te_type val) noexcept { /* simplest version of factorial */
...
[[nodiscard]]
static te_type te_sum(te_type val1, te_type val2, te_type val3, te_type val4,
                      te_type val5, te_type val6, te_type val7, te_type val8,
                      te_type val9, te_type val10, te_type val11, te_type val12,
                      te_type val13, te_type val14, te_type val15,
                      te_type val16, te_type val17, te_type val18,
                      te_type val19, te_type val20, te_type val21,
                      te_type val22, te_type val23, te_type val24) {
  const auto getSumMaybeNan = [](const auto val) {
    return (!std::isfinite(val) ? 0 : val);
  };
...
[[nodiscard]]
static te_type
te_average(te_type val1, te_type val2, te_type val3, te_type val4, te_type val5,
           te_type val6, te_type val7, te_type val8, te_type val9,
           te_type val10, te_type val11, te_type val12, te_type val13,
           te_type val14, te_type val15, te_type val16, te_type val17,
           te_type val18, te_type val19, te_type val20, te_type val21,
           te_type val22, te_type val23, te_type val24) {
  const auto isValidMaybeNan = [](const auto val) {
    return (!std::isfinite(val) ? 0 : 1);
  };
  ...
  const auto total =
      te_sum(val1, val2, val3, val4, val5, val6, val7, val8, val9, val10, val11,
             val12, val13, val14, val15, val16, val17, val18, val19, val20,
...
static te_type te_ncr(te_type val1, te_type val2) noexcept {
  ...
  for (decltype(usignR) i = 1; i <= usignR; i++) {
    if (result >
        (std::numeric_limits<uint32_t>::max)() / (usignN - usignR + i)) {
      return std::numeric_limits<te_type>::infinity();
    }
...
static te_type te_right_rotate8(te_type val1, te_type val2) {
  ...
  if (std::floor(val1) != val1 || std::floor(val2) != val2) {
    throw std::runtime_error(
        "Bitwise RIGHT ROTATE operation must use integers.");
  }
  if (val1 < 0) {
  ...
  return static_cast<te_type>(
      std::rotr(static_cast<uint8_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_left_rotate8(te_type val1, te_type val2) {
  ...
  if (std::floor(val1) != val1 || std::floor(val2) != val2) {
    throw std::runtime_error(
        "Bitwise LEFT ROTATE operation must use integers.");
  }
  if (val1 < 0) {
  ...
  return static_cast<te_type>(
      std::rotl(static_cast<uint8_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_right_rotate16(te_type val1, te_type val2) {
  ...
  return static_cast<te_type>(
      std::rotr(static_cast<uint16_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_left_rotate16(te_type val1, te_type val2) {
  ...
  return static_cast<te_type>(
      std::rotl(static_cast<uint16_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_right_rotate32(te_type val1, te_type val2) {
  ...
  return static_cast<te_type>(
      std::rotr(static_cast<uint32_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_left_rotate32(te_type val1, te_type val2) {
  ...
  return static_cast<te_type>(
      std::rotl(static_cast<uint32_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_right_rotate64(te_type val1, te_type val2) {
  ...
  return static_cast<te_type>(
      std::rotr(static_cast<uint64_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_left_rotate64(te_type val1, te_type val2) {
  ...
  return static_cast<te_type>(
      std::rotl(static_cast<uint64_t>(val1), static_cast<int>(val2)));
}
...
static te_type te_bitwise_or(te_type val1, te_type val2) {
  ...
  if (val1 < 0 || val2 < 0) {
    throw std::runtime_error(
        "Bitwise OR operation must use positive integers.");
  }
  ...
  return static_cast<te_type>(static_cast<uint64_t>(val1) |
...
static te_type te_bitwise_xor(te_type val1, te_type val2) {
  ...
  if (val1 < 0 || val2 < 0) {
    throw std::runtime_error(
        "Bitwise XOR operation must use positive integers.");
  }
  if (val1 > te_parser::MAX_BITOPS_VAL || val2 > te_parser::MAX_BITOPS_VAL) {
    throw std::runtime_error("Value is too large for bitwise operation.");
  }
...
static te_type te_bitwise_and(te_type val1, te_type val2) {
  ...
  if (val1 < 0 || val2 < 0) {
    throw std::runtime_error(
        "Bitwise AND operation must use positive integers.");
  }
  if (val1 > te_parser::MAX_BITOPS_VAL || val2 > te_parser::MAX_BITOPS_VAL) {
    throw std::runtime_error("Value is too large for bitwise operation.");
  }
...
static te_type te_left_shift(te_type val1, te_type val2) {
  ...
  // If we are limited to something like 53 bits, then we can use that (same as
  // Excel)
  ...
  // bitness is limited to 53-bit or 64-bit, so ensure shift doesn't go beyond
  // that and cause undefined behavior
  ...
  const auto multiplier =
      (static_cast<uint64_t>(1) << static_cast<uint64_t>(val2));
  const auto maxBaseNumber =
...
static te_type te_right_shift(te_type val1, te_type val2) {
  ...
  if (std::floor(val2) != val2) {
    throw std::runtime_error("Additive expression of right shift (>>) "
...
/// @warning This emulates Excel, where a negative shift amount acts as a right
/// shift.\n
///     Be aware of this if using this function outside TinyExpr++.
//--------------------------------------------------
[[nodiscard]]
static te_type te_left_shift_or_right(te_type val1, te_type val2) {
  return (val2 >= 0) ? te_left_shift(val1, val2)
                     : te_right_shift(val1, std::abs(val2));
}
...
static te_type te_right_shift_or_left(te_type val1, te_type val2) {
  return (val2 >= 0) ? te_right_shift(val1, val2)
                     : te_left_shift(val1, std::abs(val2));
}
...
[[nodiscard]]
static te_type te_max(te_type val1, te_type val2, te_type val3, te_type val4,
                      te_type val5, te_type val6, te_type val7, te_type val8,
                      te_type val9, te_type val10, te_type val11, te_type val12,
                      te_type val13, te_type val14, te_type val15,
                      te_type val16, te_type val17, te_type val18,
                      te_type val19, te_type val20, te_type val21,
                      te_type val22, te_type val23, te_type val24) noexcept {
...
[[nodiscard]]
static te_type te_min(te_type val1, te_type val2, te_type val3, te_type val4,
                      te_type val5, te_type val6, te_type val7, te_type val8,
                      te_type val9, te_type val10, te_type val11, te_type val12,
                      te_type val13, te_type val14, te_type val15,
                      te_type val16, te_type val17, te_type val18,
...
[[nodiscard]]
static te_type
te_and_variadic(te_type val1, te_type val2, te_type val3, te_type val4,
                te_type val5, te_type val6, te_type val7, te_type val8,
                te_type val9, te_type val10, te_type val11, te_type val12,
...
[[nodiscard]]
static te_type
te_or_variadic(te_type val1, te_type val2, te_type val3, te_type val4,
               te_type val5, te_type val6, te_type val7, te_type val8,
               te_type val9, te_type val10, te_type val11, te_type val12,
               te_type val13, te_type val14, te_type val15, te_type val16,
               te_type val17, te_type val18, te_type val19, te_type val20,
...
[[nodiscard]]
static te_type
te_ifs(te_type if1, te_type if1True, te_type if2, te_type if2True, te_type if3,
...
void te_parser::te_free_parameters(te_expr *texp) {
  ...
  if (is_closure(texp->m_value)) {
    ...
    for (auto param = texp->m_parameters.begin();
         param != texp->m_parameters.end() - 1; ++param) {
      te_free(*param);
      *param = nullptr;
    }
...
const std::set<te_variable> te_parser::m_functions = { // NOLINT
    ...
    {"bitlrotate8", static_cast<te_fun2>(te_builtins::te_left_rotate8),
     TE_PURE},
    {"bitrrotate8", static_cast<te_fun2>(te_builtins::te_right_rotate8),
     TE_PURE},
    {"bitlrotate16", static_cast<te_fun2>(te_builtins::te_left_rotate16),
     TE_PURE},
    {"bitrrotate16", static_cast<te_fun2>(te_builtins::te_right_rotate16),
     TE_PURE},
    {"bitlrotate32", static_cast<te_fun2>(te_builtins::te_left_rotate32),
     TE_PURE},
    {"bitrrotate32", static_cast<te_fun2>(te_builtins::te_right_rotate32),
     TE_PURE},
    {"bitlrotate64", static_cast<te_fun2>(te_builtins::te_left_rotate64),
     TE_PURE},
    {"bitrrotate64", static_cast<te_fun2>(te_builtins::te_right_rotate64),
     TE_PURE},
    ...
    {"bitlshift", static_cast<te_fun2>(te_builtins::te_left_shift_or_right),
     TE_PURE},
    {"bitrshift", static_cast<te_fun2>(te_builtins::te_right_shift_or_left),
     TE_PURE},
    ...
    {"power", /* Excel alias*/ static_cast<te_fun2>(te_builtins::te_pow),
     TE_PURE},
    ...
    {"sum", static_cast<te_fun24>(te_builtins::te_sum),
     static_cast<te_variable_flags>(TE_PURE | TE_VARIADIC)},
    {"supports32bit", static_cast<te_fun0>(te_builtins::te_supports_32bit),
     TE_PURE},
    {"supports64bit", static_cast<te_fun0>(te_builtins::te_supports_64bit),
     TE_PURE},
    ...
    if (((*theState->m_next >= '0') && (*theState->m_next <= '9')) ||
...
#ifdef TE_FLOAT
      theState->m_value =
          static_cast<te_type>(std::strtof(theState->m_next, &nEnd));
#elif defined(TE_LONG_DOUBLE)
      theState->m_value =
          static_cast<te_type>(std::strtold(theState->m_next, &nEnd));
#else
      theState->m_value =
          static_cast<te_type>(std::strtod(theState->m_next, &nEnd));
#endif
      ...
      if (is_letter(*theState->m_next) || (*theState->m_next == '_')) {
        ...
        const std::string_view currentVarToken{
            start,
            static_cast<std::string::size_type>(theState->m_next - start)};
              ...
              if (m_unknownSymbolResolve.index() == 1) {
                ...
                if (std::isfinite(retUsrVal)) {
                  ...
                  assert(m_currentVar != theState->m_lookup.cend() &&
                         "Internal error in parser using unknown symbol "
                         "resolver.");
                  if (m_currentVar != theState->m_lookup.cend()) {
        ...
        // rotate (circular shift) operators (uses the 64-bit integer version)
          ...
          theState->m_type = state::token_type::TOK_INFIX;
          theState->m_value =
              static_cast<te_fun2>(te_builtins::te_right_rotate);
          std::advance(theState->m_next, 2);
        }
#else
        ...
        } else if (tok == '!' && (*theState->m_next == '=')) // NOLINT
        {
          ...
          theState->m_type = state::token_type::TOK_INFIX;
          theState->m_value =
              static_cast<te_fun2>(te_builtins::te_less_than_equal_to);
          std::advance(theState->m_next, 1);
        } else if (tok == '<') {
          theState->m_type = state::token_type::TOK_INFIX;
          ...
          theState->m_type = state::token_type::TOK_INFIX;
          theState->m_value =
              static_cast<te_fun2>(te_builtins::te_greater_than_equal_to);
          std::advance(theState->m_next, 1);
        } else if (tok == '>') {
          theState->m_type = state::token_type::TOK_INFIX;
          theState->m_value =
              static_cast<te_fun2>(te_builtins::te_greater_than);
        } else if (tok == '&' && (*theState->m_next == '&')) {
          theState->m_type = state::token_type::TOK_INFIX;
...
te_expr *te_parser::base(state *theState) {
  ...
  if (theState->m_type == state::token_type::TOK_OPEN) {
    ...
    theState->m_type = state::token_type::TOK_ERROR;
  } else if (is_function0(theState->m_value) ||
             is_closure0(theState->m_value)) {
    ...
    if (theState->m_type != state::token_type::TOK_OPEN) {
      ...
      // If there are vars or other functions in the parameters, keep track of
      // the original opening function; that is what we will do our variadic
      // check on.
      ...
      if (theState->m_type == state::token_type::TOK_CLOSE &&
          (i != arity - 1) && varValid && is_variadic(openingVar->m_type)) {
        next_token(theState);
      } else if (theState->m_type != state::token_type::TOK_CLOSE ||
                 (i != arity - 1)) {
        theState->m_type = state::token_type::TOK_ERROR;
      } else {
        next_token(theState);
...
te_expr *te_parser::expr_level7(state *theState) {
  ...
  while (
      ...
      (get_function2(theState->m_value) == te_builtins::te_less_than ||
       ...
       get_function2(theState->m_value) ==
           te_builtins::te_greater_than_equal_to)) {
    const te_fun2 func = get_function2(theState->m_value);
    next_token(theState);
...
te_expr *te_parser::factor(te_parser::state *theState) {
  ...
  while (theState->m_type == state::token_type::TOK_INFIX &&
         is_function2(theState->m_value) &&
         (get_function2(theState->m_value) ==
          static_cast<te_fun2>(te_builtins::te_pow))) {
    ...
    if (insertion) {
      ...
      te_expr *insert =
          new_expr(TE_PURE, t, {insertion->m_parameters[1], power(theState)});
...
te_expr *te_parser::power(state *theState) {
  ...
  while (theState->m_type == state::token_type::TOK_INFIX &&
    ...
    if (is_function2(theState->m_value) &&
      ...
      theSign = -theSign;
               ...
               get_function1(theState->m_value) ==
                   te_builtins::te_bitwise_not) {
      bitwiseNot = true;
    }
    next_token(theState);
  }
  ...
  if (bitwiseNot) {
    ret = new_expr(TE_PURE, te_variant_type(te_builtins::te_bitwise_not),
                   {base(theState)});
  } else if (theSign == -1) {
    ret = new_expr(TE_PURE, te_variant_type(te_builtins::te_negate),
                   {base(theState)});
  } else {
    ret = base(theState);
  }
...
auto make_closure_arg_list(const F &func, te_expr *ctx,
...
te_type te_parser::te_eval(const te_expr *texp) {
  ...
  const auto M = [&texp = std::as_const(texp)](const size_t e) {
    return (e < texp->m_parameters.size()) ? te_eval(texp->m_parameters[e])
                                           : te_nan;
  };
  ...
  return std::visit(
      [&, texp]<typename T0>(const T0 &var) -> te_type {
        ...
        if constexpr (te_is_closure_v<T>) {
          ...
          return std::apply(var, make_closure_arg_list(
                                     M, texp->m_parameters[n_args - 1],
                                     std::make_index_sequence<n_args - 1>{}));
        }
        if constexpr (te_is_function_v<T>) {
          constexpr size_t n_args = te_function_arity<T>;
          return std::apply(var, make_function_arg_list(
                                     M, std::make_index_sequence<n_args>{}));
        }
        return te_nan;
      },
      texp->m_value);
...
//--------------------------------------------------
te_expr *te_parser::te_compile(const std::string_view expression,
...
bool te_parser::compile(const std::string_view expression) {
  ...
  try {
    m_compiledExpression =
        te_compile(m_expression, get_variables_and_functions());
...
te_type te_parser::evaluate() {
  try {
    ...
    m_result = (m_compiledExpression != nullptr) ? te_eval(m_compiledExpression)
                                                 : te_nan;
  } catch (const std::exception &exp) {
    m_parseSuccess = false;
    m_result = te_nan;
...
//--------------------------------------------------
te_type te_parser::evaluate(
    const std::string_view expression) // NOLINT(-readability-identifier-naming)
{
...
std::string te_parser::info() {
  ...
  sysInfo +=
      "^, &, | operators:        bitwise XOR, bitwise AND, and bitwise OR\n";
#else
  sysInfo +=
      "^, &, | operators:        exponentiation, logical AND, and logical OR\n";
#endif
#ifdef TE_BRACKETS_AS_PARENS
